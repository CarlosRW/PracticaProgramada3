...
public class ListDoublyCircular<T> implements ListInterface<T> {
    ...
    /**
     * Agrega un nuevo nodo al inicio (head) de la lista.
     */
    public void addFirst(T value) {
        ...
        if (isEmpty()) {
            head = newNode;
            tail = newNode;
            head.next = head; // Un solo nodo se apunta a sí mismo
            head.prev = head;
        } else {
            newNode.next = head;
            newNode.prev = tail;
            head.prev = newNode;
            tail.next = newNode;
            head = newNode; // El nuevo nodo es la nueva cabeza
        }
    }

    /**
     * Agrega un nuevo nodo al final (tail) de la lista.
     */
    public void addLast(T value) {
        ...
        if (isEmpty()) {
            ...
            head.prev = newNode;
            tail.next = newNode;
            tail = newNode; // El nuevo nodo es la nueva cola
        }
    }

    /**
     * Elimina el primer nodo (head) de la lista y retorna su valor.
    ...
    public T removeFirst() {
        ...
        if (head == tail) { // Lista con un solo elemento
            head = null;
            tail = null;
        } else { // Lista con múltiples elementos
            NodeDoubly<T> nextHead = head.next;
            tail.next = nextHead;
            nextHead.prev = tail;
            head = nextHead; // La nueva cabeza es el siguiente
        }
        return data;
    }

    /**
     * Elimina el último nodo (tail) de la lista y retorna su valor.
     * Retorna null si la lista está vacía.
     */
    public T removeLast() {
        if (isEmpty()) {
        ...
        if (head == tail) { // Lista con un solo elemento
            ...
            newTail.next = head;
            head.prev = newTail;
            tail = newTail; // La nueva cola es el anterior
        }
        return data;
    }
